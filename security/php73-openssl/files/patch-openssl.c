--- openssl.c.orig	2020-03-17 10:18:41 UTC
+++ openssl.c
@@ -18,6 +18,7 @@
...
+#include <openssl/ocsp.h>
 ...
 /* Common */
 #include <time.h>
@@ -130,6 +132,56 @@ PHP_FUNCTION(openssl_pkey_derive);
...
@@ -495,12 +547,20 @@ static const zend_function_entry openssl
...
@@ -749,6 +809,7 @@ void php_openssl_store_errors()
 static int le_key;
 static int le_x509;
 static int le_csr;
+static int le_crl;
...
@@ -757,6 +818,16 @@ int php_openssl_get_x509_list_id(void) /
 }
 /* }}} */
...
+
 ...
 {
@@ -778,6 +849,21 @@ static void php_openssl_csr_free(zend_re
...
@@ -1423,6 +1509,7 @@ PHP_MINIT_FUNCTION(openssl)
...
+	le_crl = zend_register_list_destructors_ex(php_openssl_crl_free, NULL, "OpenSSL X.509 CRL", module_number);
 	...
 	OPENSSL_config(NULL);
@@ -1525,6 +1612,36 @@ PHP_MINIT_FUNCTION(openssl)
...
+static int check_cert(X509_STORE *ctx, X509 *x, STACK_OF(X509) *untrustedchain, int purpose, long flags)
 {
 	int ret=0;
 	X509_STORE_CTX *csc;
@@ -2535,6 +2652,9 @@ static int check_cert(X509_STORE *ctx, X
...
@@ -2548,6 +2668,59 @@ static int check_cert(X509_STORE *ctx, X
 }
 /* }}} */
...
@@ -2583,7 +2756,7 @@ PHP_FUNCTION(openssl_x509_checkpurpose)
 		goto clean_exit;
 	}
...
@@ -6874,6 +7047,442 @@ PHP_FUNCTION(openssl_random_pseudo_bytes
 }
 /* }}} */
