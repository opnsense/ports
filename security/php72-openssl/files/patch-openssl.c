--- openssl.c.orig	2019-03-05 11:06:04 UTC
+++ openssl.c
@@ -18,6 +18,7 @@
...
+#include <openssl/ocsp.h>
 ...
 /* Common */
 #include <time.h>
@@ -131,6 +133,56 @@ PHP_FUNCTION(openssl_dh_compute_key);
...
@@ -490,12 +542,20 @@ const zend_function_entry openssl_functi
...
@@ -743,6 +803,7 @@ void php_openssl_store_errors()
 static int le_key;
 static int le_x509;
 static int le_csr;
+static int le_crl;
...
@@ -751,6 +812,16 @@ int php_openssl_get_x509_list_id(void) /
 }
 /* }}} */
...
+
 ...
 {
@@ -772,6 +843,21 @@ static void php_openssl_csr_free(zend_re
...
@@ -1417,6 +1503,7 @@ PHP_MINIT_FUNCTION(openssl)
...
+	le_crl = zend_register_list_destructors_ex(php_openssl_crl_free, NULL, "OpenSSL X.509 CRL", module_number);
 	...
 	OPENSSL_config(NULL);
@@ -1519,6 +1606,36 @@ PHP_MINIT_FUNCTION(openssl)
...
+static int check_cert(X509_STORE *ctx, X509 *x, STACK_OF(X509) *untrustedchain, int purpose, long flags)
 {
 	int ret=0;
 	X509_STORE_CTX *csc;
@@ -2526,6 +2643,9 @@ static int check_cert(X509_STORE *ctx, X
...
@@ -2539,6 +2659,59 @@ static int check_cert(X509_STORE *ctx, X
 }
 /* }}} */
...
@@ -2574,7 +2747,7 @@ PHP_FUNCTION(openssl_x509_checkpurpose)
 		goto clean_exit;
 	}
...
@@ -6801,6 +6974,442 @@ PHP_FUNCTION(openssl_random_pseudo_bytes
 }
 /* }}} */
